
undefined8 FUN_1014f4078(void)

{
  ulong *puVar1;
  long lVar2;
  ulong uVar3;
  long *plVar4;
  ulong *unaff_x19;
  ulong uVar5;
  ulong *puVar6;
  ulong in_xzr;
  
  FUN_1014a10ac(&stack0x00001470);
  lVar2 = *(long *)unaff_x19[0x20];
  *(long *)unaff_x19[0x20] = lVar2 + -1;
  LORelease();
  if (lVar2 == 1) {
    DataMemoryBarrier(2,1);
    func_0x0001060e2224(unaff_x19 + 0x20);
  }
  puVar6 = unaff_x19 + 0x1f;
  uVar5 = *puVar6;
  *(undefined1 *)((long)unaff_x19 + 0xbb) = 0;
  LOAcquire();
  lVar2 = *(long *)(uVar5 + 0x1f0);
  *(long *)(uVar5 + 0x1f0) = lVar2 + -1;
  LORelease();
  if (lVar2 == 1) {
    LOAcquire();
    lVar2 = *(long *)(uVar5 + 0x88);
    *(long *)(uVar5 + 0x88) = lVar2 + 1;
    lVar2 = func_0x000100fcbaa8(uVar5 + 0x80,lVar2);
    *(ulong *)(lVar2 + 0xb10) = *(ulong *)(lVar2 + 0xb10) | 0x200000000;
    LORelease();
    puVar1 = (ulong *)(uVar5 + 0x110);
    LOAcquire();
    uVar3 = *puVar1;
    *puVar1 = uVar3 | 2;
    LORelease();
    if (uVar3 == 0) {
      lVar2 = *(long *)(uVar5 + 0x100);
      *(undefined8 *)(uVar5 + 0x100) = 0;
      *puVar1 = in_xzr;
      LORelease();
      if (lVar2 != 0) {
        (**(code **)(lVar2 + 8))(*(undefined8 *)(uVar5 + 0x108));
      }
    }
  }
  lVar2 = *(long *)*puVar6;
  *(long *)*puVar6 = lVar2 + -1;
  LORelease();
  if (lVar2 == 1) {
    DataMemoryBarrier(2,1);
    __ZN5alloc4sync16Arc_LT_T_C_A_GT_9drop_slow17h49f814765505db59E(puVar6);
  }
  *(undefined1 *)((long)unaff_x19 + 0xbc) = 0;
  lVar2 = *(long *)unaff_x19[0x1e];
  *(long *)unaff_x19[0x1e] = lVar2 + -1;
  LORelease();
  if (lVar2 == 1) {
    DataMemoryBarrier(2,1);
    __ZN5alloc4sync16Arc_LT_T_C_A_GT_9drop_slow17h55ac392a03c3ae0cE();
  }
  *(undefined1 *)((long)unaff_x19 + 0xbd) = 0;
  if (unaff_x19[0x24] != 0) {
    _free(unaff_x19[0x22]);
  }
  lVar2 = *(long *)unaff_x19[0x15];
  *(long *)unaff_x19[0x15] = lVar2 + -1;
  LORelease();
  if (lVar2 == 1) {
    DataMemoryBarrier(2,1);
    __ZN5alloc4sync16Arc_LT_T_C_A_GT_9drop_slow17h55ac392a03c3ae0cE();
  }
  lVar2 = *(long *)unaff_x19[0x14];
  *(long *)unaff_x19[0x14] = lVar2 + -1;
  LORelease();
  if (lVar2 == 1) {
    DataMemoryBarrier(2,1);
    func_0x0001060e2224();
  }
  puVar6 = unaff_x19 + 0x13;
  uVar5 = *puVar6;
  LOAcquire();
  lVar2 = *(long *)(uVar5 + 0x1f0);
  *(long *)(uVar5 + 0x1f0) = lVar2 + -1;
  LORelease();
  if (lVar2 == 1) {
    LOAcquire();
    lVar2 = *(long *)(uVar5 + 0x88);
    *(long *)(uVar5 + 0x88) = lVar2 + 1;
    lVar2 = func_0x000100fcbaa8(uVar5 + 0x80,lVar2);
    *(ulong *)(lVar2 + 0xb10) = *(ulong *)(lVar2 + 0xb10) | 0x200000000;
    LORelease();
    puVar1 = (ulong *)(uVar5 + 0x110);
    LOAcquire();
    uVar3 = *puVar1;
    *puVar1 = uVar3 | 2;
    LORelease();
    if (uVar3 == 0) {
      lVar2 = *(long *)(uVar5 + 0x100);
      *(undefined8 *)(uVar5 + 0x100) = 0;
      *puVar1 = in_xzr;
      LORelease();
      if (lVar2 != 0) {
        (**(code **)(lVar2 + 8))(*(undefined8 *)(uVar5 + 0x108));
      }
    }
  }
  lVar2 = *(long *)*puVar6;
  *(long *)*puVar6 = lVar2 + -1;
  LORelease();
  if (lVar2 == 1) {
    DataMemoryBarrier(2,1);
    __ZN5alloc4sync16Arc_LT_T_C_A_GT_9drop_slow17h49f814765505db59E(puVar6);
  }
  if ((char)unaff_x19[0x11] != '\0') {
    lVar2 = *(long *)unaff_x19[0x12];
    *(long *)unaff_x19[0x12] = lVar2 + -1;
    LORelease();
    if (lVar2 == 1) {
      DataMemoryBarrier(2,1);
      __ZN5alloc4sync16Arc_LT_T_C_A_GT_9drop_slow17h10f445fb86fd931aE();
    }
  }
  if (unaff_x19[0xf] == 0) {
    plVar4 = (long *)unaff_x19[0x10];
    lVar2 = *plVar4;
    *plVar4 = lVar2 + -1;
    LORelease();
    if (lVar2 == 1) {
      DataMemoryBarrier(2,1);
      __ZN5alloc4sync16Arc_LT_T_C_A_GT_9drop_slow17h0f68883db9c1e126E();
    }
  }
  else {
    plVar4 = (long *)unaff_x19[0x10];
    lVar2 = *plVar4;
    *plVar4 = lVar2 + -1;
    LORelease();
    if (lVar2 == 1) {
      DataMemoryBarrier(2,1);
      __ZN5alloc4sync16Arc_LT_T_C_A_GT_9drop_slow17h576a4a0a079aa50eE();
    }
  }
  *(undefined1 *)((long)unaff_x19 + 0xb7) = 1;
  if (*unaff_x19 != 2) {
    uVar5 = unaff_x19[1];
    if ((*unaff_x19 & 1) != 0) {
      uVar5 = uVar5 + (*(long *)(unaff_x19[2] + 0x10) - 1U & 0xfffffffffffffff0) + 0x10;
    }
    (**(code **)(unaff_x19[2] + 0x68))(uVar5,unaff_x19 + 3);
  }
  if ((__ZN12tracing_core10dispatcher6EXISTS17h9b1f5f531bc653aaE == '\0') && (unaff_x19[4] != 0)) {
    FUN_1011d7d74();
  }
  return 0;
}

