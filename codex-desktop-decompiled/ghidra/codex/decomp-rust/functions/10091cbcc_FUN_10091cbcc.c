
void FUN_10091cbcc(undefined8 param_1)

{
  long unaff_x19;
  long *unaff_x20;
  long unaff_x21;
  long unaff_x25;
  long unaff_x26;
  long unaff_x29;
  
  *(char **)(unaff_x29 + -0x70) = s__failed_to_get_token_for_auth_st_108acb481;
  *(long *)(unaff_x29 + -0x68) = unaff_x29 + -0x60;
  *(long *)(unaff_x29 + -0x80) = unaff_x29 + -0x70;
  *(undefined **)(unaff_x29 + -0x78) = &UNK_10b208fd0;
  *(undefined8 *)(unaff_x29 + -0xa0) = 1;
  *(long *)(unaff_x29 + -0x98) = unaff_x29 + -0x80;
  *(undefined8 *)(unaff_x29 + -0x90) = 1;
  *(undefined8 *)(unaff_x29 + -0x88) = param_1;
  __ZN7tracing15__macro_support13__tracing_log17h72768b16bee39960E();
  FUN_100de21d4(unaff_x29 + -0xc0);
  FUN_100e0609c(&stack0x000000a0);
  *(undefined1 *)(unaff_x19 + 0x12) = 1;
  FUN_100d5b0c4();
  *unaff_x20 = unaff_x25 + 0x65;
  unaff_x20[1] = -0x8000000000000000;
  unaff_x20[2] = unaff_x21;
  unaff_x20[3] = unaff_x26;
  unaff_x20[4] = 0x401;
  *(undefined1 *)(unaff_x20 + 5) = 1;
  *(undefined1 *)((long)unaff_x20 + 0x29) = 4;
  *(undefined1 *)(unaff_x19 + 0xfa8) = 1;
  return;
}

